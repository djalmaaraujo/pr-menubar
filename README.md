<p align="center">
  <img src="assets/logo.svg" alt="PR Bar" width="480">
</p>
...
<p align="center">
  <img src="assets/screenshot-main.png" alt="PR Bar popover listing open pull requests with state, repo, and CI status" width="480">
</p>
...
You already know your open PRs — `gh search prs --author=@me --state=open` lists them. PR Bar just runs that for you every 60 seconds and keeps the result one click away in the menu bar.
...
- **Filter by org or account** — a dropdown auto-populated from the orgs you can access, plus your personal account and an "All" option. Orgs with private membership still show up, because PR Bar also picks up any owner that appears in your actual PRs.
...
Compiles, packages `PRBar.app`, ad-hoc signs it, and opens it. Drag `build/PRBar.app` into `/Applications` to keep it around.

## How it works

PR Bar makes two kinds of `gh` calls and never talks to GitHub any other way.
...
Nothing leaves your machine except the GitHub API calls `gh` already makes on your behalf, with the credentials `gh` already holds. PR Bar has no token, no server, no analytics, and stores nothing beyond your selected filter in `UserDefaults`.
